...
function AddShop() {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/shops/upload`, {
      method: "POST",
      body: formData,
    });
    if (!res.ok) throw new Error('Upload failed');
    const data = await res.json();
    // Lưu URL thật vào state
    setShopInfo((prev) => ({ ...prev, image: data.url }));
  } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    // chuẩn bị dữ liệu gửi đúng key backend
    const dataToSend = {
      name: shopInfo.name,
      ...
      imgStore: shopInfo.image, // URL tạm thời
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "Lỗi khi lưu salon");
      }

      alert("Đã lưu thông tin salon thành công!");
      ...
      alert("Lỗi khi lưu salon: " + err.message);
    }
  };
    ...
    <div className="flex-1 overflow-y-auto bg-surface p-8">
      <section className="sp-8 space-y-8 mb-10">
        ...
        <p className="text-on-surface-variant font-body max-w-xl italic border-l-2 border-primary-container pl-4">
          "Nơi vẻ đẹp và nghệ thuật giao thoa để tạo nên những trải nghiệm dịch vụ đẳng cấp nhất."
        </p>
      </section>
